...
function getSnappedPos(gps: [number,number], coords: any[]): [number,number] {
    ...
    return minD < 25 ? best : gps; // Only snap if within 25m
}

export const cleanInstruction = (text: string): string => {
  if (!text) return '';
  let r = text.replace(/\u2019|\u0027/g, "'").replace(/(Prenez la direction|Head|Se diriger vers (l'|le |la )|Direction|Vers (l'|le |la ))(nord|sud|est|ouest|nord-est|nord-ouest|sud-est|sud-ouest)\s*(sur\s*)?(la\s+|le\s+|l')?/ig, 'CONTINUEZ SUR ').toUpperCase();
  return r;
};
...
export default function Map({
  ...
  useEffect(() => {
    ...
    fetch(`https://router.project-osrm.org/route/v1/driving/${center[1]},${center[0]};${selectedAlert.lng},${selectedAlert.lat}?overview=full&geometries=geojson&steps=true`, {signal:abort.signal})
  ...
  useEffect(() => {
    ...
    if (autoCenter) {
        ...
        if (navigationActive) {
            p=45; zoom = speed*3.6<20 ? 20 : 18.5;
        ...
        // Update arrow rotation inside element
    ...
    alerts.forEach(a => {
        if (!alertMarkers.current[a.id]) {
            const el = document.createElement('div'); el.className = styles.alertMarker;
            el.innerHTML = a.type==='fire'?'🔥':'🚑';
  ...
  useEffect(() => {
    if (route?.legs[0]?.steps?.length > 0) {
        ...
        setGuidance({ text: cleanInstruction(step.maneuver.instruction), dist: Math.round(step.distance) });
    }
  }, [route]);
  ...
  return (
    <div className={styles.mapWrapper}>
      ...
      <style>{`
        ...
        .v-arrow-svg { width: 40px; height: 40px; transition: transform 0.1s linear; }
      `}</style>
    </div>
  );
}
